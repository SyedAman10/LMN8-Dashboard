...
export default function AddCollegeModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        onSave(data.college, data);
        onClose();
      } else {
        setErrors({ general: data.error || 'Failed to create college' });
      }
    } catch (error) {
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-slate-800/95 backdrop-blur-xl border border-cyan-500/30 rounded-2xl shadow-2xl flex flex-col">
        <div className="bg-gradient-to-r from-slate-700/50 to-slate-600/50 p-6 border-b border-slate-600/30 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-white">Add New College</h2>
              <p className="text-slate-300">Create a college with a dashboard account</p>
            </div>
